import { GET_INCOMES_URL, GET_INCOME_BY_ID_URL, ADD_INCOME_URL, UPDATE_INCOME_URL, DELETE_INCOME_URL } from './constants';

export const Incomes = {
    getIncomes: async (limit = 5, offset = 0, sortBy = 'id') => {        
        const response = await fetch(`${GET_INCOMES_URL}?limit=${limit}&offset=${offset}&sortBy=${sortBy}`);
        if (!response.ok) {
